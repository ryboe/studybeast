...
# -v /tmpkey.json:/key.json:ro -- The file provisioner will copy the service account key file to /key.json
...
docker run --rm -p 127.0.0.1:5432:3306 -v /tmp/svc_account_key.json:/key.json:ro gcr.io/cloudsql-docker/gce-proxy:latest /cloud_sql_proxy -credential_file=/key.json -ip_address_types=PRIVATE -instances=${db_instance_name}=tcp:0.0.0.0:3306
